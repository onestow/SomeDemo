...
namespace LeetCode.problems
{
    public class _410 : BaseClass
    {
        ...
        public int SplitArray(int[] nums, int m)
        {
            ...
            var dp = new long[nums.Length + 1, m + 1];
            for (int i = 1; i <= nums.Length; i++)
                for (int j = 1; j <= m && j <= i; j++)
                {
                    ...
                    else
                    {
                        ...
                        for (int k = j - 1; k < i; k++)
                            t = Math.Min(t, Math.Max(dp[k, j - 1], sums[i] - sums[k]));
                        dp[i, j] = t;
                    }
                }
